...
class ZabbixSender(object):
    ...
    def __init__(self,
        ...
        if use_config:
            self.zabbix_uri = self._load_from_config(use_config, path)
        else:
    ...
    def _load_from_config(self, config_file, file_path):
    ...
    def _chunk_send(self, metrics):
        ...
        for host_addr in self.zabbix_uri:
            ...
            try:
                ...
                connection.close()
                raise Exception('Sending failed: Connection to '+ host_addr[0] + ":" + str(host_addr[1]) + ' timed out after '+ str(self.timeout) + ' seconds')
            except Exception as err:
                # In case of error we should close connection, otherwise
                # we will close it afret data will be received.
                logger.warn('Sending failed: %s', err.args)
                connection.close()
                raise Exception('Sending failed: ' + err.args[1])
            ...
            if response and response.get('response') != 'success':
                ...
                raise Exception('Response error: ' + response)
